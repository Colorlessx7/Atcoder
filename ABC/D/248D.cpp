...
int main(){
  ...
  //is[i][j]:iがxに対応 jがAi=xなるiのインデックスの集合
  ...
  rep(i,q){
    ...
    //引数のisはAi=xのインデックスiだけをまとめた配列のみ引数とする
    //参照により無駄なコピーを回避する
    auto f = [&](vector<ll> &is, ll r){
      //二分探索 rより大きい配列aの値の添字(is[x][j]のj)から
      //.begin()(j=0)をひくと区間[0,r]のxの個数が得られる
...
//is[x].end()のインデックス2がf(r)の値になる
//f(l-1)はl-1=-1なのでより大きい1がでるインデックス0がf(l-1)となる 
//よってf(r)-f(l-1)=2-0=2となる

//こういう問題をsegtreeでやる場合は(a[i],i)で座標圧縮してセグ木を作る
//やり方だと更新に対応したりできる
//また、is[a[i]]をBinary Indexed Treeにする方法もある
